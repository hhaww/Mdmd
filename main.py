...
def start(message):
	...
	if f == SUDO:
		...
		M = types.InlineKeyboardButton('DEV', url='https://t.me/v_a_c')
		mas.add(G,K)
		mas.add(V)
		mas.add(M)
		...
		me = types.InlineKeyboardButton(text="حدو",url="https://t.me/v_a_c")
		he = types.InlineKeyboardButton(text="زين",url="https://t.me/Z_E_1")
		
		ch = types.InlineKeyboardButton(text="▶ قناة البوت ◀",url="https://t.me/teshkir")
		rr.add(me,he,de)
		rr.add(ch)
...
def masg(call):
	...
	if call.data =="v_a_c":
		...
		while True:
			...
			v_a_c = [h1,h2,h3,h4,h5]
			j = random.choice(v_a_c)
			url = "https://t.me/"+j
			headers = {
            "User-Agent": generate_user_agent(),
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8",
			...
			if response.text.find('If you have <strong>Telegram</strong>, you can contact <a class="tgme_username_link"')>=0 and j not in ban:
				...
				R = types.InlineKeyboardButton(f'{sk}', callback_data="1x")
				v_a_c = types.InlineKeyboardButton('DEV', url='https://t.me/v_a_c')
				mas.add(A,E,B,R,v_a_c)
				bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text="ok start",reply_markup=mas)
